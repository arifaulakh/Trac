...
export const handler: APIGatewayProxyHandler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
		...
		const podcastName = parsedBody.name.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '');
    ...
    return {
      ...
      body: JSON.stringify({ message: `Transcript of episode-${uploadParams.podcastId}-${uploadParams.podcastName} will be uploaded to s3 bucket ${uploadParams.bucket}`}),
    };
    ...
    if (error.response) {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'No response received from the server.' })
      };
    } else {
      ...
      return {
        statusCode: 500,
        body: JSON.stringify({ error: 'An error occurred while setting up the request.' })
      };
    }
  }
};
